...
function AiPanel({tutors,anagraficaCorsi,avvisi=[],settings,user,isSuperAdmin,tutEvents={},corsi=[],onAddTut,onAddCorso,onOverwriteTut,onOverwriteCorso,onSaveTutor,onSaveAnaCorso,onSaveAvviso,onOpenAnaTutors,onOpenAnaCorsi,onClose}){
  ...
  async function analyzeDoc(file,text){const prov=settings.aiProvider||"openai";const prompt=`Estrai TUTTI gli appuntamenti dal documento. Anno di riferimento: 2026. Rispondi SOLO con JSON valido (nessun testo extra):\n{"events":[{"day":8,"month":"mag-26","start":9.5,"end":13.5,"ore":4,"name":"nome corso"}],"ambiguities":[]}\nRegole:\n- month: abbreviazione italiana 3 lettere + anno 2 cifre (gen feb mar apr mag giu lug ago set ott nov dic)\n- start/end: ore decimali (es 9.5=09:30, 14.25=14:15)\n- ore: durata in ore (end-start)\n- Estrai assolutamente TUTTI gli appuntamenti senza saltarne nessuno. Inserisci in ambiguities solo i dati davvero incerti.`;
