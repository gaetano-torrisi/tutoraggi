...
function AiPanel({tutors,anagraficaCorsi,settings,user,isSuperAdmin,onAddTut,onAddCorso,onSaveTutor,onSaveAnaCorso,onOpenAnaTutors,onOpenAnaCorsi,onClose}){
  ...
  function renderMsg(msg,idx){const isAi=msg.role==="ai";
    return(<div key={idx} style={{display:"flex",flexDirection:"column",alignItems:isAi?"flex-start":"flex-end",marginBottom:10}}>
      ...
      {msg.showOpenAv&&<button onClick={onOpenAnaCorsi} className="btn" data-variant="outline" style={{marginTop:6,display:"flex",alignItems:"center",gap:6}}><Icon name="briefcase" size={13}/>Apri Anagrafica Avvisi</button>}
  ...
  return(<div className="add-modal-backdrop">
    <div className="add-modal-box">
      ...
      {tab==="tutoraggio"&&(<>
        ...
        :<select value={tutAvId} onChange={e=>{if(e.target.value==="__open__"){onClose();onOpenAnaCorsi();}else setTutAvId(e.target.value);}} className="select" style={{marginBottom:10}}>{[...anagraficaCorsi].sort((a,b)=>a.nome.localeCompare(b.nome)).map(a=><option key={a.id} value={a.id}>{a.nome}</option>)}<option value="__open__">+ Apri Anagrafica Avvisi...</option></select>}
