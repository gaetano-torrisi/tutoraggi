    ...
    <div className="detail-pane" style={{overflowY:"auto"}}>
      ...
      :selected?(<div style={{maxWidth:520}}>
        <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:24}}>
          ...
          <div><h2 style={{fontSize:17,fontWeight:700,color:"var(--fg)",marginBottom:4,wordBreak:"break-all"}}>{selected.email}{isSelf&&<span style={{marginLeft:8,fontSize:11,padding:"2px 7px",borderRadius:100,background:"var(--accent-soft)",color:"var(--accent-strong)",fontWeight:700,verticalAlign:"middle"}}>il tuo account</span>}</h2><div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}><span className="badge" data-tone={selected.role==="superadmin"?"danger":selected.role==="admin"?"accent":selected.role==="viewer"?"default":"info"}>{ROLE_LABEL[selected.role]||selected.role}</span>{canEditProfile&&!profileLoading&&<span className="badge" data-tone={(isSelf||profileUid)?"success":"warning"} style={{display:"inline-flex",alignItems:"center",gap:4}}><Icon name={(isSelf||profileUid)?"checkCircle":"clock"} size={11} color="currentColor"/>{(isSelf||profileUid)?"Utente creato e registrato":"Utente creato, in attesa di registrazione"}</span>}</div></div>
        </div>
        ...
        {canEditProfile&&<>
          ...
          :(!isSelf&&!profileUid)?<div style={{padding:"10px 14px",borderRadius:"var(--radius)",background:"var(--bg-sunken)",border:"1px solid var(--border)",fontSize:13,color:"var(--fg-muted)",marginBottom:16,display:"flex",alignItems:"center",gap:8}}><Icon name="user" size={14} color="var(--fg-faint)"/>I dati personali saranno compilabili dopo il primo accesso dell'utente.</div>
          :<>
