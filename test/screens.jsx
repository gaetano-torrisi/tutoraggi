...
function runVerifica(corsi,anagraficaCorsi,tutors,tutEvents){
  ...
  // ore_incoerente — slot/sessioni dove il campo "ore" non corrisponde a (fine − inizio): falsa il totale ore mostrato nelle KPI
  for(const[tid,months]of Object.entries(tutEvents)){for(const{mk,evs}of safeEvs(months)){const t=tutors.find(x=>x.id===tid);for(const ev of evs){if(ev.end<=ev.start)continue;const der=ev.end-ev.start;const mem=typeof ev.ore==="number"?ev.ore:NaN;if(isNaN(mem)||Math.abs(mem-der)>0.001)errors.push({type:"ore_incoerente",monthKey:mk,evId:ev.id,day:ev.day,msg:`Tutor "${t?.cognome} ${t?.nome}": slot "${ev.name||"?"}" del ${fmtDayMonth(ev.day,mk)} ha ore=${isNaN(mem)?"non impostato":fmtOreMin(mem)} ma l'orario ${fmt(ev.start)}–${fmt(ev.end)} vale ${fmtOreMin(der)}.`,detail:`Il totale ore conteggia ${isNaN(mem)?"0h":fmtOreMin(mem)} invece di ${fmtOreMin(der)} (scarto ${fmtOreMin(Math.abs((isNaN(mem)?0:mem)-der))}). Riapri e salva lo slot per ricalcolare le ore.`});}}}
  for(const ana of anagraficaCorsi){const co=corsiById[ana.id];if(!co)continue;for(const ev of (co.events||[])){const mk=ev.month||"";if(ev.end<=ev.start)continue;const der=ev.end-ev.start;const mem=typeof ev.ore==="number"?ev.ore:NaN;if(isNaN(mem)||Math.abs(mem-der)>0.001)errors.push({type:"ore_incoerente",monthKey:mk,evId:ev.id,day:ev.day,msg:`Corso "${ana.nome}": sessione del ${fmtDayMonth(ev.day,mk)} ha ore=${isNaN(mem)?"non impostato":fmtOreMin(mem)} ma l'orario ${fmt(ev.start)}–${fmt(ev.end)} vale ${fmtOreMin(der)}.`,detail:`Il totale ore conteggia ${isNaN(mem)?"0h":fmtOreMin(mem)} invece di ${fmtOreMin(der)} (scarto ${fmtOreMin(Math.abs((isNaN(mem)?0:mem)-der))}). Riapri e salva la sessione per ricalcolare le ore.`});}}
  // tutor_senza_slot
...
const VERIFICA_CATS=[
  ...
  {type:"ore_incoerente",label:"Ore slot incoerenti",icon:"clock",tone:"warning"},
];
