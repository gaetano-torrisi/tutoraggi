      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px 24px",background:"var(--bg)"}}>
        ...
        {viewMode==="avviso"&&[...anagraficaAv].filter(a=>!selAvFilter||a.nome===selAvFilter).sort((a,b)=>a.nome.localeCompare(b.nome)).map(ana=>{
          ...
          return(<div key={ana.id} style={{marginBottom:8,borderRadius:"var(--radius-md)",border:"1px solid var(--border)",overflow:"hidden",background:"var(--bg-elev)"}}>
            ...
            {exp&&<div style={{padding:"8px 14px",borderTop:"1px solid var(--divider)"}}>
              {ana.durataOre&&pct!=null&&(()=>{const tutOreAv=tutors.reduce((s,t)=>s+getTutOreAvPeriodo(t.id,ana.nome),0);const tutPct=Math.round(tutOreAv/ana.durataOre*100);return(<div style={{marginBottom:10,paddingBottom:10,borderBottom:"1px solid var(--divider)"}}>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:10,fontWeight:700,color:"var(--fg-subtle)",textTransform:"uppercase",letterSpacing:".06em"}}>Avanzamento ore</span><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:9,color:"var(--fg-subtle)",fontStyle:"italic"}}>{periodSubtitle()}</span><span style={{fontFamily:'"JetBrains Mono",monospace',fontSize:12,fontWeight:700,color:tutPct>100?"var(--danger)":"var(--fg)"}}>{tutPct}% · {fmtOreMin(tutOreAv)}/{fmtOreMin(ana.durataOre)}</span></div></div>
                <div className="progress-bar-track" style={{height:6}}><div className="progress-bar-fill" style={{width:`${Math.min(100,tutPct)}%`,background:`linear-gradient(90deg,${ana.colore||"var(--accent)"}66,${ana.colore||"var(--accent)"}99)`}}/></div>
              </div>);})()}
